...
app.post('/api/collect/start', async (req, res) => {
  ...
  const { mode, resume } = req.body; // mode: full, issues, meta, audit; resume: boolean
  collectionState = {
    running: true,
    phase: 'starting',
    progress: null,
  ...
  runCollection(mode || 'full', { resume: !!resume });
});
...
async function runCollection(mode, { resume = false } = {}) {
  ...
  try {
    ...
    if (mode === 'audit') {
      ...
      setPhase('issues');
      await collectors.collectIssues(client, db, jql, { resume });
    } else if (mode === 'meta') {
      setPhase('metadata');
